...
export class CelebComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.celebService.search({
                query: this.currentSearch,
                page: this.page,
                size: this.itemsPerPage,
                sort: this.sort()
            }).subscribe(
                (res: HttpResponse<Celeb[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: Celeb) {
        return item.id;
    }
    registerChangeInCelebs() {
        this.eventSubscriber = this.eventManager.subscribe('celebListModification', (response) => this.reset());
    }
