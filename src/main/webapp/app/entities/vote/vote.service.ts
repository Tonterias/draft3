...
export class VoteService {
    ...
    search(req?: any): Observable<HttpResponse<Vote[]>> {
        const options = createRequestOption(req);
        return this.http.get<Vote[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Vote[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertItemFromServer(vote: Vote): Vote {
        ...
        copy.creationDate = this.dateUtils
            .convertDateTimeFromServer(vote.creationDate);
        return copy;
    }
    ...
    private convert(vote: Vote): Vote {
        ...
        copy.creationDate = this.dateUtils.toDate(vote.creationDate);
        return copy;
    }
}
